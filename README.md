...
The training set was assembled incrementally through production use and error-driven retraining, and naturally covers diverse capture conditions: front-facing and oblique angles, indoor and outdoor lighting, varying distances, and two different camera types (dedicated digital camera and smartphone).

**On the ~4% real-world errors:** almost all failures are edge cases — extreme values rarely seen during training (e.g. sub-10 kg readings), severe backlighting, or unusual shooting angles. Critically, these failures are *predictable and detectable*: the misread value typically differs from the correct one by an order of magnitude, making them easy to catch during business review. Collecting these edge-case photos and adding them to the training set resolves each failure class permanently. This is precisely what the error-driven retraining loop is designed for. In practice, several thousand photos exist in the production deployment and are available for future labeling; only ~300 were annotated and used here to validate that the pipeline works at minimal scale. The strong generalization observed on held-out historical photos suggests the architecture is not the bottleneck — scaling the dataset is the straightforward path to pushing real-world accuracy beyond 96%.
